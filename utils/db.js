...
class Database {
  ...
  async getGuildSettings(guildId) {
    return new Promise((resolve, reject) => {
      this.db.get(
        ...
        (err, row) => {
          if (err) reject(err);
          else resolve(row || { guild_id: guildId, level_up_messages: true });
        }
      );
    });
  }
  ...
  async updateGuildSettings(guildId, settings) {
    return new Promise((resolve, reject) => {
      this.db.run(
        `INSERT OR REPLACE INTO guild_settings (guild_id, level_up_messages) 
         VALUES (?, ?)`,
        [guildId, settings.level_up_messages],
        function(err) {
          if (err) reject(err);
          else resolve();
        }
