  ...
  async execute(interaction, client) {
    ...
    if (subcommand === 'remove') {
      ...
      try {
        ...
        if (userData.background && userData.background.startsWith('./data/backgrounds/')) {
          const backgroundPath = path.join(__dirname, '..', userData.background);
    ...
    if (subcommand === 'view') {
      ...
      try {
        ...
        await interaction.editReply({
          content: '❌ Failed to load your background. It may have been corrupted.',
        });
      }
    }
  },
